 ...
 */
?>
<div id="learndash_complete_prerequisites" class="learndash_complete_prerequisites"><?php 
	echo sprintf( 
	 esc_html_x(
	...
	if ( !empty( $prerequisite_posts_all ) ) {
		foreach( $prerequisite_posts_all as $pre_post_id => $pre_status ) {
			if ( $pre_status === false ) {
				if ( !empty( $post_links ) ) $post_links .= '<br/>';
				$post_links .= '<div class="arrow_right green"></div><a href="'. get_the_permalink( $pre_post_id ) .'">'. get_the_title( $pre_post_id ) .'</a>';
			}
		}
	}
