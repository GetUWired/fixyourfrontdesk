...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		jQuery("#learndash_next_prev_link > a.next-link").html("Next Lesson<div class='arrow_right'></div>");

		jQuery("#learndash_next_prev_link > a.prev-link").html("<div class='arrow_left'></div>Previous Lesson");
		...
		<?php
			...
			if ((!empty($previous_item)) && ($previous_item instanceof WP_Post)) {
				if ($previous_item->post_type == 'sfwd-quiz') {
					...
					echo sprintf(
						esc_html_x('Please go back and complete the previous %s.', 'placeholders: topic URL, topic label', 'learndash'),
					...
					echo sprintf(
						esc_html_x('Please go back and complete the previous %s.', 'placeholders: lesson URL, lesson label', 'learndash'),
...
<?php if ($show_content) : ?>
	...
	<?php if ((isset($materials)) && (!empty($materials))) : ?>
		<div id="learndash_lesson_materials" class="learndash_lesson_materials">
			<!-- 			<h4><?php //printf( esc_html_x( '%s Materials', 'Lesson Materials Label', 'learndash' ), LearnDash_Custom_Label::get_label( 'lesson' ) ); 
											?></h4> -->
			<h4><span></span>Your Downloads for <?php echo the_title(); ?> <div class="arrow_down"></div>
			</h4>
